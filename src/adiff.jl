...
#
Grad(v::T)             where T        = Grad(tuple(v)) 
Grad(v::Array{T})      where T        = Grad(tuple(v...))
D2(v::T)               where T        = D2(v,Grad(one(T)))
D2{N}(v::T)            where {N,T}    = D2(v,zero(Grad{N,T}),zero(Grad{(N+1)N/2,T}))
D2{N,M,T}(v::Number)   where {N,M,T}  = D2(T(v),zero(Grad{N,T}),zero(Grad{(N+1)N/2,T}))
...
D1(v::T)               where T        = D1(v,Grad(one(T)))
D1(x::Array{T})        where T  = begin
  N     = length(x)
  grad  = init(Grad{N,T})
  [D2(x, grad[ii], zero(Grad{(N+1)N/2,T})) for (ii,x) in enumerate(x)]
end
D1(x::D2)                     = D1(x.v, x.g)
D2(x::D1{N,T}) where {N,T}    = D2(x.v, x.g, zero(Grad{(N+1)N/2,T})) 
# promotion
...
@inline grad(x::D1)             = [x.g[i]   for i in 1:N]
@inline hess(x::D1)             = zeros(N,N)
@inline val(U::Array{D1})       = [u.v for u in U]
# @inline grad(U::Array{D1})      = [[u.g[ii] for u in U] for ii=1:N]
# @inline hess(U::Array{D1})      = [[u.h[ii] for u in U] for ii=1:M]
# D2 operators
